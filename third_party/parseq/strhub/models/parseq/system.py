...
class PARSeq(CrossEntropySystem):
    ...
    def forward(self, images: Tensor, max_length: Optional[int] = None) -> Tensor:
        ...
        if self.refine_iters:
            ...
            # masked_fill_ avoids a PyTorch 1.9 CUDA boolean-indexing assertion while
            # preserving the exact cloze mask used by the official implementation.
            query_mask.masked_fill_(
                torch.triu(torch.ones(num_steps, num_steps, dtype=torch.bool, device=self._device), 2), 0
            )
